...
class MainWindow(tk.Tk):
    ...
    def check(self, *args):
        if (self.r1.get()=='Гугл' or self.r1.get()=='Яндекс') and (self.r2.get()=='US' or self.r2.get()=='RUS') and self.scale.get():
...
class ChildWindow2(tk.Toplevel):
    def __init__(self, parent):
        ...
        # self.search_label = tk.Label(self, text='Поиск')
        # self.search_label.pack()
        #
        # self.search_entry = tk.Entry(self)
        # self.search_entry.pack()
        #
        # self.search_button = tk.Button(self, text='Найти', command=self.search)
        ...
        # self.tree.tag_configure('not_matched', background='white')

    # def search(self):
    #     choosed = self.search_entry.get().lower()
    #     for item in self.tree.get_children():
    #         if choosed in self.tree.item(item)['values'][0].lower():
    #             self.tree.item(item, tags='matched')
    #         else:
    #             self.tree.item(item, tags='not_matched')
