...
class MainWindow(tk.Tk):
    def __init__(self):
        ...
        self.box = ttkb.Frame(self, padding=7)
        self.box.grid(column=1, row=2, rowspan=6)

        self.level_box = ttkb.Frame(self, padding=0)
        self.level_box.grid(column=2, columnspan=3, row=2, rowspan=6)

        self.level = ttkb.Frame(self.level_box)
        self.level.grid(column=2,columnspan=3,row=3,rowspan=1)
        ...
        self.password_lb = ttkb.Label(self.box, text='Выберите длину пароля', foreground="white")
        self.password_lb.grid(column=1,row=3, pady=10,sticky='w')

        self.scale = ttkb.Scale(self.level_box, from_=8, to=100, orient='horizontal', command=self.scale_update)
        self.scale.grid(column=2,row=2, pady=0, sticky='nsew', padx=20)

        self.scale_label = ttkb.Label(self.level_box, text='')
        self.scale_label.grid(column=2,row=1)

        # ЧЕКБОКС!!!!!!!!

        self.check_level = ttkb.Label(self.box, text='Выберите уровень', foreground="white")
        self.check_level.grid(column=1,row=4, pady=12, sticky='w')



        self.var = IntVar()
        self.button1 = ttkb.Radiobutton(self.level, text='Простой', variable=self.var, value=0)
        self.button1.grid(column=2,row=4, sticky='w', padx=20, pady=10)
        self.button2 = ttkb.Radiobutton(self.level, text='Средний', variable=self.var, value=1)
        self.button2.grid(column=3,row=4, sticky='w', padx=0, pady=10)
        self.button3 = ttkb.Radiobutton(self.level, text='Сложный', variable=self.var, value=2)
        ...
        self.mail_lb.grid(column=1,row=5, pady=14, sticky='w')

        items = ["Гугл", "Яндекс"]

        self.combobox_mail = ttkb.Combobox(self.level_box, values=items, textvariable=self.r1)
        self.combobox_mail.grid(column=2,row=5, sticky='w', pady=8, padx=30, columnspan=2)

        self.country_lb = ttkb.Label(self.box, text='Страна', foreground="white")
        self.country_lb.grid(column=1,row=6, pady=8, sticky='w')
        countries = ['US', 'RUS']
        self.combobox_country = ttkb.Combobox(self.level_box, values=countries, textvariable=self.r2)
        self.combobox_country.grid(column=2,row=6, pady=8, padx=0, columnspan=2)

        self.password = ttkb.Button(self, text='Сгенерировать', command=self.open, state='disabled', width=20)
        self.password.grid(column=1,row=8, pady=35, columnspan=2, rowspan=2, ipady=5, ipadx=5, sticky='e')

        self.save = ttkb.Button(self, text='Сохранённые записи', command=self.open1)
        self.save.grid(column=0,row=10, padx=5, pady=0, sticky='ws', columnspan=2)
...
class ChildWindow(tk.Toplevel):
    def __init__(self, parent, scale_value=8, var_value=1, combobox_male_value='uuu',
        ...
        self.entry_box = ttkb.Frame(self,padding=10)
        self.entry_box.grid(column=1, columnspan=3, rowspan=6, padx=10, pady=5)
        ...
        self.btn_close.grid(column=4,row=0)

        self.dbsave = ttkb.Button(self, text='Сохранить', command=self.save)
        self.dbsave.grid(column=1,row=6, sticky='nw', padx=15)

        self.again = ttkb.Button(self, text='Сгенерировать заново', command=self.generate)
        self.again.grid(column=3,row=6, columnspan=1, sticky='nw')
...
class ChildWindow2(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.name = ttkb.Label(self, text='Учетные записи')
        self.name.pack(pady=8)

        self.tree = ttkb.Treeview(self, height=13)
        ...
        self.popup1 = ttkb.Menu(self, tearoff=False)
